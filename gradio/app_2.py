...
def build_models(model_type, config, enable_optimization=False):
    ...
    vae_id = 'hpcai-tech/OpenSora-VAE-v1.2'
    vae_2d = dict(
        type="VideoAutoencoderKL",
        from_pretrained="PixArt-alpha/pixart_sigma_sdxlvae_T5_diffusers",
        subfolder="vae",
        micro_batch_size=config.vae.micro_batch_size,
    ...
    kwargs = dict(
        ...
        freeze_vae_2d=False,
        cal_loss=False,
        micro_frame_size=config.vae.micro_frame_size,
        shift=shift,
        scale=scale,
    )
    ...
    text_encoder = T5Encoder(from_pretrained=text_encoder_id, dtype=dtype, model_max_length=300, t5_load_mode=t5_load_mode)
    # build text encoder
...
from opensora.models.text_encoder.t5 import T5Encoder, text_preprocessing
...
def run_inference(
    ...
    with torch.cuda.amp.autocast(dtype=dtype, enabled=True) and torch.inference_mode():
        # ======================
